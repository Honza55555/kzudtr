#!/usr/bin/env python3
# main.py – funguje na Python 3.13+ bez externích stub souborů, jediný soubor

import sys
import types

# ————————————————————————————————————————————————————————————————
# 1) Stub modulu imghdr pro Python 3.13+, aby python-telegram-bot nenarážel na
#    chybějící standardní knihovnu
# ————————————————————————————————————————————————————————————————
...
RESPONSES = {
    ...
    "hours_cz":   CZ_HOURS,
    "where_cz":   CZ_WHERE,
    "contact_cz": CZ_CONTACT,
    "preorder_cz":CZ_PREORDER,
    "reasons_cz": CZ_REASONS,
    "menu_en":    EN_MENU,
    "hours_en":   EN_HOURS,
    "where_en":   EN_WHERE,
    "contact_en": EN_CONTACT,
    "preorder_en":EN_PREORDER,
    "reasons_en": EN_REASONS,
}
...
# ————————————————————————————————————————————————————————————————
# FLASK HEALTH-CHECK (Render free-tier)
# ————————————————————————————————————————————————————————————————
app = Flask(__name__)
...
def button(update, context: CallbackContext):
    ...
    if key.startswith("lang_"):
        ...
        q.edit_message_text(
            RESPONSES.get(key, "❓ Omlouvám se, něco se pokazilo."),
            parse_mode=ParseMode.HTML
        )
...
def main():
    ...
    # spustíme Flask health-check server
    threading.Thread(target=run_health, daemon=True).start()

    # spustíme Telegram polling
